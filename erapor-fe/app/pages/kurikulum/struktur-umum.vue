        ...
        <div class="flex-1 overflow-y-auto custom-scrollbar p-5">
            ...
            <form v-else @submit.prevent="saveData" class="space-y-4">
                ...
                <div>
                    ...
                    <select v-model="formData.kelompok_kode" required class="w-full px-4 py-3 rounded-2xl border-2 border-slate-200/70 bg-slate-50 focus:bg-white focus:ring-4 focus:ring-amber-500/10 focus:border-amber-500 transition-all text-sm font-bold text-slate-800 outline-none cursor-pointer">
                ...
                <div>
                    ...
                    <input type="number" v-model="formData.jp" required min="1" max="50" placeholder="Contoh: 2" class="w-full px-4 py-3 rounded-2xl border-2 border-slate-200/70 bg-slate-50 focus:bg-white focus:ring-4 focus:ring-amber-500/10 focus:border-amber-500 transition-all text-sm font-bold text-slate-800 outline-none">
                </div>
...
const fetchData = async () => {
    ...
    const token = useCookie('auth_token').value
    try {
        const url = `${import.meta.env.VITE_API_BASE_URL}/api/kurikulum/struktur-umum` + (selectedKurikulumId.value ? `?kurikulum_id=${selectedKurikulumId.value}` : '')
...
const saveData = async () => {
    ...
    const token = useCookie('auth_token').value
    try {
        const response = await $fetch(import.meta.env.VITE_API_BASE_URL + '/api/kurikulum/struktur-umum', {
            method: 'POST',
            headers: { Authorization: `Bearer ${token}` },
            body: {
...
const executeDelete = async () => {
    ...
    const token = useCookie('auth_token').value
    try {
        const response = await $fetch(`${import.meta.env.VITE_API_BASE_URL}/api/kurikulum/struktur-umum/${deleteTarget.value.id}`, {
            method: 'DELETE',
            headers: { Authorization: `Bearer ${token}` }
        })
        if (response.success) {
