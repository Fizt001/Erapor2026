    ...
    <div class="flex-1 flex overflow-hidden relative">
      ...
      <div class="xl:hidden absolute top-0 left-0 w-full bg-white border-b border-slate-200 flex-shrink-0 p-2 grid grid-cols-2 gap-2 z-20">
        <button v-for="tab in mobileTabs" :key="'mob-'+tab.id" type="button" @click="activeTab = tab.id"
          :class="activeTab === tab.id ? 'bg-gradient-to-br from-indigo-500 to-indigo-600 text-white shadow-md shadow-indigo-500/20 ring-2 ring-indigo-500 ring-offset-1' : 'bg-white text-slate-500 shadow-sm border border-slate-100'"
        ...
        <div class="p-6 shrink-0 z-10 relative">
          <div class="bg-gradient-to-r from-indigo-600 to-blue-700 rounded-2xl p-5 border border-indigo-500 shadow-sm relative overflow-hidden flex items-center gap-4">
            ...
            <div class="relative z-10">
                ...
                <p class="text-[10px] text-indigo-100 font-semibold uppercase mt-0.5">{{ isEdit ? 'Perbarui Data' : 'Tambah Data Manual' }}</p>
            </div>
        ...
        <div class="flex-1 overflow-y-auto custom-scrollbar p-5">
            <form @submit.prevent="saveData" class="space-y-4">
                <div>
                    ...
                    <select v-model="form.kurikulum_id" required class="w-full px-4 py-3 rounded-2xl border-2 border-slate-200/70 bg-slate-50 focus:bg-white focus:ring-4 focus:ring-indigo-500/10 focus:border-indigo-500 transition-all text-sm font-bold text-slate-700 outline-none">
                ...
                <div>
                    ...
                    <select v-model="form.nama_periode" required class="w-full px-4 py-3 rounded-2xl border-2 border-slate-200/70 bg-slate-50 focus:bg-white focus:ring-4 focus:ring-indigo-500/10 focus:border-indigo-500 transition-all text-sm font-bold text-slate-700 outline-none">
                ...
                <div class="grid grid-cols-2 gap-4">
                    <div>
                        ...
                        <input v-model="form.tempat_cetak" type="text" required placeholder="Misal: Bekasi" class="w-full px-4 py-3 rounded-2xl border-2 border-slate-200/70 bg-slate-50 focus:bg-white focus:ring-4 focus:ring-indigo-500/10 focus:border-indigo-500 transition-all text-sm font-bold text-slate-700 outline-none">
                    </div>
                    <div>
                        <label class="block text-[11px] font-black text-slate-500 uppercase mb-1.5 ml-1">Tanggal Cetak</label>
                        <input v-model="form.tanggal_cetak" type="date" required class="w-full px-4 py-3 rounded-2xl border-2 border-slate-200/70 bg-slate-50 focus:bg-white focus:ring-4 focus:ring-indigo-500/10 focus:border-indigo-500 transition-all text-sm font-bold text-slate-700 outline-none">
                    </div>
                </div>
                ...
                <div class="pt-4 border-t border-slate-100 flex gap-3">
                    ...
                    <button type="submit" :disabled="isSaving" class="flex-[2] py-3 bg-gradient-to-r from-indigo-500 to-indigo-600 text-white font-bold rounded-2xl shadow-lg shadow-indigo-500/30 hover:-translate-y-0.5 transition-all flex items-center justify-center gap-2">
                        <span v-if="isSaving" class="animate-spin">⏳</span>
                        <span v-else>{{ isEdit ? '💾' : '➕' }}</span> 
      ...
      <div :class="['flex-1 bg-slate-50 flex flex-col h-full min-w-0', activeTab === 'table' || isDesktop ? 'flex' : 'hidden', !isDesktop ? 'pt-[60px]' : '']">
        ...
        <div class="px-6 py-5 border-b border-slate-100 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 shrink-0 z-10 sticky top-0 bg-white/80 backdrop-blur-xl">
            <div class="flex items-center gap-4">
                ...
                <div>
                    <h3 class="text-sm font-black uppercase tracking-widest text-indigo-700">Database Titimangsa</h3>
                ...
                <tbody class="text-xs font-medium text-slate-700 divide-y divide-slate-100">
                    ...
                    <tr v-for="(item, index) in titimangsas" :key="item.id" class="hover:bg-slate-50/80 transition-colors bg-white group">
                        ...
                        <td class="p-4">
                            ...
                            <div class="text-[10px] font-bold text-indigo-500 uppercase tracking-widest mt-0.5">Tingkat: {{ item.target_tingkat }}</div>
                        </td>
                        <td class="p-4">
                        ...
                        <td class="p-4 text-center">
                            <div class="flex items-center justify-center gap-2 opacity-100 lg:opacity-0 lg:group-hover:opacity-100 transition-opacity">
                                <button @click="editData(item)" class="w-8 h-8 rounded-xl bg-white border border-slate-200 text-slate-400 hover:border-indigo-200 hover:bg-indigo-50 hover:text-indigo-600 flex items-center justify-center transition-colors shadow-sm" title="Edit">✏️</button>
                                <button @click="confirmDelete(item)" class="w-8 h-8 rounded-xl bg-white border border-slate-200 text-slate-400 hover:border-rose-200 hover:bg-rose-50 hover:text-rose-600 flex items-center justify-center transition-colors shadow-sm" title="Hapus">🗑️</button>
                            </div>
                        </td>
                    </tr>
                </tbody>
