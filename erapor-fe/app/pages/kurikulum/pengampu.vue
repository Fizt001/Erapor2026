        ...
        <div class="p-6 shrink-0 z-10 relative">
          <div class="bg-gradient-to-r from-indigo-600 to-blue-700 rounded-2xl p-5 border border-indigo-500 shadow-sm relative overflow-hidden flex items-center gap-4">
            ...
            <div class="relative z-10">
                ...
                <p class="text-[10px] text-indigo-100 font-semibold uppercase mt-0.5">Pilih Kriteria Mapel</p>
            </div>
        ...
        <div class="flex-1 overflow-y-auto px-6 pb-6 space-y-6">
            ...
            <div class="space-y-4">
                <div>
                    ...
                    <select v-model="selectedKurikulumId" @change="fetchPengampu" class="w-full px-4 py-3 rounded-2xl border-2 border-slate-200/70 bg-slate-50 focus:bg-white focus:ring-4 focus:ring-indigo-500/10 focus:border-indigo-500 transition-all text-sm font-bold text-slate-700 outline-none">
                ...
                <div>
                    ...
                    <select v-model="selectedTingkat" @change="fetchPengampu" class="w-full px-4 py-3 rounded-2xl border-2 border-slate-200/70 bg-slate-50 focus:bg-white focus:ring-4 focus:ring-indigo-500/10 focus:border-indigo-500 transition-all text-sm font-bold text-slate-700 outline-none">
                        <option v-for="tk in refTingkatKelas" :key="tk.kode" :value="tk.kode">{{ tk.nama }}</option>
                    </select>
                </div>
            </div>

            <button @click="fetchPengampu" class="w-full py-4 bg-indigo-600 hover:bg-indigo-700 active:scale-[0.98] rounded-2xl text-white shadow-lg shadow-indigo-200 transition-all flex items-center justify-center gap-2 group">
      ...
      <div class="flex-1 flex flex-col min-w-0 bg-slate-50 relative h-full" :class="activeTabMobile === 'flow' || isDesktop ? 'flex' : 'hidden'">
            ...
            <div class="p-4 bg-white border-b border-slate-200 flex justify-between items-center gap-4 shrink-0 z-10 shadow-sm">
                <div class="flex items-center gap-4">
                    ...
                    <div>
                        <h3 class="text-sm font-black uppercase tracking-widest text-indigo-700">Plot Guru Mapel</h3>
                ...
                <div v-else class="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-2 2xl:grid-cols-3 gap-6 items-start">
                    ...
                    <div v-for="struktur in strukturs" :key="struktur.id" class="bg-white border border-slate-200 rounded-2xl shadow-sm flex flex-col overflow-hidden hover:shadow-md transition-all">
                        ...
                        <div class="p-4 flex-1 flex flex-col gap-4">
                            <div v-for="kelas in kelases" :key="kelas.id" class="border border-slate-100 rounded-xl p-3 bg-white hover:border-indigo-100 transition-colors shadow-sm">
                                ...
                                <div v-if="getSisaJp(struktur, kelas.id) > 0" class="flex items-center gap-1.5 pt-2 border-t border-dashed border-slate-200">
                                    ...
                                    <input type="number" min="1" :max="getSisaJp(struktur, kelas.id)" v-model="formAssign[struktur.id + '_' + kelas.id].jp" class="w-12 py-1.5 px-2 text-[10px] font-bold rounded-lg border border-slate-200 bg-white text-center focus:ring-2 focus:ring-indigo-500 outline-none shrink-0" />
                                    <button @click="assignGuru(struktur, kelas.id)" class="w-7 h-7 rounded-lg bg-indigo-600 text-white flex items-center justify-center hover:bg-indigo-700 active:scale-90 transition-all shrink-0 disabled:opacity-50" :disabled="isSaving[struktur.id + '_' + kelas.id]">
