<template>
  <div class="h-full flex flex-col min-h-0 bg-slate-50">
    <div class="flex-1 flex flex-col xl:flex-row overflow-hidden relative">
      ...
      <!-- Panel Flow Kanan -->
      <div :class="['flex-1 bg-slate-50 flex flex-col h-full min-w-0 relative', activeTabMobile === 'cases' || isDesktop ? 'flex' : 'hidden', !isDesktop ? 'pt-[52px]' : '']">
...
const fetchKasus = async () => {
    ...
    try {
        const data = await $fetch<any>(import.meta.env.VITE_API_BASE_URL + '/api/kepsek/kasus-siswa', {
...
const submitAcc = async () => {
    ...
    const token = useCookie('auth_token').value
    try {
        const data = await $fetch<any>(`${import.meta.env.VITE_API_BASE_URL}/api/kepsek/kasus-siswa/${selectedKasus.value.id}/acc`, {
            method: 'PUT',
            headers: { Authorization: `Bearer ${token}` },
            body: form.value
        })
