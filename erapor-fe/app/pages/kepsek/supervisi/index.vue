        ...
        <div class="p-4 pb-2 shrink-0 z-10 relative">
          <div class="bg-gradient-to-r from-purple-600 to-fuchsia-600 rounded-2xl p-4 border border-purple-500 shadow-sm relative overflow-hidden flex items-center gap-3">
            ...
            <div class="relative z-10">
                ...
                <p class="text-[10px] text-purple-100 font-semibold uppercase mt-0.5">{{ isEditing ? 'Berikan Penilaian' : 'Buat Supervisi Guru' }}</p>
            </div>
        ...
        <div class="flex-1 overflow-y-auto custom-scrollbar p-4 pb-6">
            <form @submit.prevent="saveData" class="space-y-4">
                ...
                <div v-if="!isEditing">
                    ...
                    <select v-model="formData.guru_id" required class="w-full px-4 py-3 rounded-2xl border-2 border-slate-200/70 bg-slate-50 focus:bg-white focus:ring-4 focus:ring-purple-500/10 focus:border-purple-500 transition-all text-sm font-bold text-slate-800 outline-none appearance-none">
                ...
                <div class="grid grid-cols-2 gap-3">
                    <div>
                        ...
                        <input type="date" v-model="formData.tanggal" required class="w-full px-4 py-3 rounded-2xl border-2 border-slate-200/70 bg-slate-50 focus:bg-white focus:ring-4 focus:ring-purple-500/10 focus:border-purple-500 transition-all text-sm font-bold text-slate-800 outline-none">
                    </div>
                    <div>
                        <label class="block text-[11px] font-black text-slate-500 uppercase mb-1.5 ml-1">Waktu (Jam)</label>
                        <input type="time" v-model="formData.waktu" required class="w-full px-4 py-3 rounded-2xl border-2 border-slate-200/70 bg-slate-50 focus:bg-white focus:ring-4 focus:ring-purple-500/10 focus:border-purple-500 transition-all text-sm font-bold text-slate-800 outline-none">
                    </div>
                </div>

                <div>
                    <label class="block text-[11px] font-black text-slate-500 uppercase mb-1.5 ml-1">Persiapan / Keterangan</label>
                    <textarea v-model="formData.keterangan" rows="2" required placeholder="Contoh: RPP, Silabus, dan Media..." class="w-full px-4 py-3 rounded-2xl border-2 border-slate-200/70 bg-slate-50 focus:bg-white focus:ring-4 focus:ring-purple-500/10 focus:border-purple-500 transition-all text-sm font-bold text-slate-800 placeholder:text-slate-400 outline-none resize-none"></textarea>
                </div>
                ...
                <template v-if="isEditing">
                  <div class="pt-2">
                    <div class="flex items-center gap-2 mb-3">
                      ...
                      <span class="text-[9px] font-black uppercase tracking-widest text-purple-500 bg-purple-50 px-2 py-1 rounded-lg">Hasil Supervisi</span>
                      <div class="h-px bg-slate-200 flex-1"></div>
                    </div>
                    ...
                    <div class="space-y-4">
                      <div>
                          ...
                          <textarea v-model="formData.evaluasi" rows="2" placeholder="Tuliskan evaluasi di sini..." class="w-full px-4 py-3 rounded-2xl border-2 border-slate-200/70 bg-slate-50 focus:bg-white focus:ring-4 focus:ring-purple-500/10 focus:border-purple-500 transition-all text-sm font-bold text-slate-800 placeholder:text-slate-400 outline-none resize-none"></textarea>
                      </div>

                      <div>
                          <label class="block text-[11px] font-black text-slate-500 uppercase mb-1.5 ml-1">Tindak Lanjut</label>
                          <textarea v-model="formData.tindak_lanjut" rows="2" placeholder="Catatan tindak lanjut..." class="w-full px-4 py-3 rounded-2xl border-2 border-slate-200/70 bg-slate-50 focus:bg-white focus:ring-4 focus:ring-purple-500/10 focus:border-purple-500 transition-all text-sm font-bold text-slate-800 placeholder:text-slate-400 outline-none resize-none"></textarea>
                      </div>

                      <div>
                          <label class="block text-[11px] font-black text-slate-500 uppercase mb-1.5 ml-1">Status</label>
                          <select v-model="formData.status" required class="w-full px-4 py-3 rounded-2xl border-2 border-slate-200/70 bg-slate-50 focus:bg-white focus:ring-4 focus:ring-purple-500/10 focus:border-purple-500 transition-all text-sm font-bold text-slate-800 outline-none appearance-none">
                ...
                <div class="pt-4 border-t border-slate-100 flex gap-3">
                    ...
                    <button type="submit" :disabled="isSaving" class="flex-[2] py-3 bg-gradient-to-r from-purple-500 to-purple-600 text-white font-bold rounded-2xl shadow-lg shadow-purple-500/30 hover:-translate-y-0.5 transition-all flex items-center justify-center gap-2 text-xs uppercase tracking-widest">
      ...
      <div :class="['flex-1 bg-slate-50 flex flex-col h-full min-w-0 relative', activeTabMobile === 'table' || isDesktop ? 'flex' : 'hidden', !isDesktop ? 'pt-[52px]' : '']">
        ...
        <div class="px-6 py-5 border-b border-slate-100 flex flex-row justify-between items-center gap-2 shrink-0 z-10 bg-white">
            <div class="flex items-center gap-4">
                <div class="w-10 h-10 rounded-2xl bg-purple-50 shadow-sm border border-purple-100 flex items-center justify-center text-xl hidden sm:flex text-purple-500"><AppIcon name="clipboard-document-list" class="w-6 h-6" /></div>
                <div>
                    <h3 class="text-[11px] sm:text-sm font-black uppercase tracking-widest text-purple-700">Jadwal & Riwayat Supervisi</h3>
                    ...
                    <tr v-if="!supervisiList || supervisiList.length === 0">
                     ...
                     <tr v-for="(item, index) in supervisiList" :key="item.id" class="border-b border-slate-100 hover:bg-slate-50/80 transition-colors bg-white group flex flex-col sm:table-row p-4 sm:p-0 relative">
                        ...
                        <td class="px-0 py-1 sm:p-4 flex flex-col sm:table-cell mb-2 sm:mb-0 w-full gap-1 align-top">
                            <div class="flex items-start justify-between w-full">
                                ...
                                <span class="px-2 py-0.5 rounded text-[9px] font-black uppercase tracking-widest border"
                                  :class="{
                                      'bg-purple-100 text-purple-700 border-purple-200': item.status === 'Terjadwal',
                        ...
                        <td class="px-0 py-1 sm:p-4 flex flex-col sm:table-cell mb-2 sm:mb-0 w-full gap-2 align-top">
                            ...
                            <div v-if="item.evaluasi" class="mt-2 text-[10px] text-slate-600 bg-purple-50/50 p-2 rounded-lg border border-purple-100">
                                <span class="font-bold text-purple-400 uppercase tracking-widest block mb-0.5 text-[9px]">Evaluasi:</span> 
                                {{ item.evaluasi }}
                            </div>
