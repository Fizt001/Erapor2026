...
<!-- Layout 2 Panel Dock & Flow -->
    <div class="flex-1 flex overflow-hidden relative">
      ...
      <!-- Panel Dock Kiri -->
      <div :class="['w-full xl:w-[360px] bg-white border-r border-slate-200 flex-shrink-0 flex flex-col h-full z-10 shadow-[2px_0_10px_-4px_rgba(0,0,0,0.05)] transition-all overflow-y-auto custom-scrollbar', activeTab === 'form' || isDesktop ? 'block' : 'hidden xl:flex', !isDesktop ? 'pt-[52px]' : '']">
        
        <div class="p-4 pb-2 space-y-4 animate-fadeIn">
                                ...
                                <tr v-for="(item, index) in group" :key="item.id" class="border-b border-slate-100 hover:bg-emerald-50/30 transition-colors group bg-white flex flex-col sm:table-row p-4 sm:p-0 relative">
                                    ...
                                    <td class="px-0 pt-2 sm:p-4 sm:pr-6 text-center border-t sm:border-0 border-slate-100 mt-2 sm:mt-0 flex sm:table-cell justify-end w-full sm:w-32">
                                        <div class="flex items-center justify-end sm:justify-center gap-2 opacity-100 lg:opacity-0 lg:group-hover:opacity-100 transition-opacity w-full">
                                            ...
                                            <button @click="confirmDelete(item)" class="w-8 h-8 rounded-lg bg-white border border-slate-200 text-slate-400 hover:text-rose-500 hover:bg-rose-50 hover:border-rose-200 flex items-center justify-center transition-all shadow-sm" title="Hapus">
