    ...
    <div class="flex-1 flex overflow-hidden relative">
      ...
      <div class="w-full xl:w-[360px] bg-white border-r border-slate-200 flex-shrink-0 flex flex-col h-full z-10 shadow-[2px_0_10px_-4px_rgba(0,0,0,0.05)] transition-all hidden xl:flex">
        ...
        <div class="p-6 shrink-0 relative z-10">
          <div class="bg-gradient-to-r from-violet-600 to-indigo-700 rounded-2xl p-5 border border-violet-500 shadow-sm relative overflow-hidden flex items-center gap-4">
            <div class="w-10 h-10 flex items-center justify-center text-2xl shrink-0 relative z-10">👤</div>
            <div class="relative z-10">
                <h3 class="text-sm font-black uppercase tracking-widest text-white">Profil Siswa</h3>
                <p class="text-[10px] text-violet-100 font-semibold uppercase mt-0.5">Identitas Siswaistrator</p>
            </div>
          ...
          <div class="bg-white rounded-3xl shadow-sm border border-slate-200/60 overflow-hidden flex flex-col flex-1 relative min-h-0">
            ...
            <div class="px-6 py-5 border-b border-slate-100 flex items-center justify-between sticky top-0 bg-white/80 backdrop-blur-xl z-20">
                <div class="flex items-center gap-4">
                    ...
                    <div>
                        <h3 class="text-sm font-black uppercase tracking-widest text-indigo-700">Data Akun Siswa</h3>
                        <p class="text-[10px] font-bold text-slate-400 uppercase mt-0.5">Informasi Kredensial Login</p>
                    </div>
                </div>
                <button @click="saveProfile" :disabled="isSaving" class="px-6 py-2.5 bg-indigo-500 hover:bg-indigo-600 text-white font-bold rounded-xl shadow-md disabled:opacity-70 flex items-center justify-center gap-2 text-xs uppercase tracking-widest transition-all">
            ...
            <div class="flex-1 overflow-y-auto custom-scrollbar p-6 md:p-10 relative z-0">
                <div class="animate-fadeIn space-y-8">
                    ...
                    <div class="flex flex-col sm:flex-row items-center sm:items-start gap-6 border-b border-slate-100 pb-8">
                        <div class="w-24 h-24 rounded-3xl bg-gradient-to-br from-indigo-500 to-violet-600 shadow-lg shadow-indigo-500/30 flex items-center justify-center text-white text-4xl font-black border-4 border-white shrink-0">
                            {{ userInitials }}
                        </div>
                        <div class="text-center sm:text-left pt-2">
                            <h2 class="text-2xl font-black text-slate-800">{{ userProfile?.name || 'Siswaistrator' }}</h2>
                            <p class="text-sm font-semibold text-indigo-600 tracking-wider uppercase mt-1">{{ userProfile?.role || 'Siswa' }} Sistem Erapor</p>
