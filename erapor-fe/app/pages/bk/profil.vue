    ...
    <div class="flex-1 flex overflow-hidden relative">
      ...
      <div class="w-full xl:w-[360px] bg-white border-r border-slate-200 flex-shrink-0 flex flex-col h-full z-10 shadow-[2px_0_10px_-4px_rgba(0,0,0,0.05)] transition-all hidden xl:flex">
        ...
        <div class="p-6 shrink-0 relative z-10">
          <div class="bg-gradient-to-r from-pink-600 to-rose-700 rounded-2xl p-5 border border-pink-500 shadow-sm relative overflow-hidden flex items-center gap-4">
            <div class="w-10 h-10 flex items-center justify-center text-2xl shrink-0 relative z-10">👤</div>
            <div class="relative z-10">
                <h3 class="text-sm font-black uppercase tracking-widest text-white">Profil BK</h3>
                <p class="text-[10px] text-pink-100 font-semibold uppercase mt-0.5">Identitas BKistrator</p>
            </div>
        ...
        <div class="flex-1 overflow-y-auto p-4 space-y-2 custom-scrollbar">
            <button 
                type="button" 
                class="w-full flex items-center gap-4 p-4 rounded-2xl transition-all duration-300 text-left relative group overflow-hidden border bg-rose-50 border-rose-200 text-rose-700"
            >
                <div class="w-10 h-10 rounded-xl flex items-center justify-center text-xl transition-all duration-300 shrink-0 bg-white text-rose-600 shadow-sm">
                    🛡️
                </div>
          ...
          <div class="bg-white rounded-3xl shadow-sm border border-slate-200/60 overflow-hidden flex flex-col flex-1 relative min-h-0">
            ...
            <div class="px-6 py-5 border-b border-slate-100 flex items-center justify-between sticky top-0 bg-white/80 backdrop-blur-xl z-20">
                <div class="flex items-center gap-4">
                    <div class="w-12 h-12 rounded-2xl bg-gradient-to-br from-rose-400 to-rose-600 shadow-sm flex items-center justify-center text-2xl text-white">
                        🛡️
                    </div>
                    <div>
                        <h3 class="text-sm font-black uppercase tracking-widest text-rose-700">Data Akun BK</h3>
                        <p class="text-[10px] font-bold text-slate-400 uppercase mt-0.5">Informasi Kredensial Login</p>
                    </div>
                </div>
                <button @click="saveProfile" :disabled="isSaving" class="px-6 py-2.5 bg-rose-500 hover:bg-rose-600 text-white font-bold rounded-xl shadow-md disabled:opacity-70 flex items-center justify-center gap-2 text-xs uppercase tracking-widest transition-all">
            ...
            <div class="flex-1 overflow-y-auto custom-scrollbar p-6 md:p-10 relative z-0">
                <div class="animate-fadeIn space-y-8">
                    ...
                    <div class="flex flex-col sm:flex-row items-center sm:items-start gap-6 border-b border-slate-100 pb-8">
                        <div class="w-24 h-24 rounded-3xl bg-gradient-to-br from-rose-500 to-pink-600 shadow-lg shadow-rose-500/30 flex items-center justify-center text-white text-4xl font-black border-4 border-white shrink-0">
                            {{ userInitials }}
                        </div>
                        <div class="text-center sm:text-left pt-2">
                            <h2 class="text-2xl font-black text-slate-800">{{ userProfile?.name || 'BKistrator' }}</h2>
                            <p class="text-sm font-semibold text-rose-600 tracking-wider uppercase mt-1">{{ userProfile?.role || 'BK' }} Sistem Erapor</p>
                    ...
                    <form @submit.prevent="saveProfile">
                        <h4 class="text-[11px] font-black text-slate-400 uppercase tracking-widest mb-4 border-l-4 border-rose-500 pl-3">Rincian Profil</h4>
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            ...
                            <div>
                                ...
                                <div class="relative">
                                    ...
                                    <input type="text" v-model="form.name" required class="w-full pl-11 pr-4 py-3 rounded-2xl border-2 border-slate-200/70 bg-slate-50 focus:bg-white focus:ring-4 focus:ring-rose-500/10 focus:border-rose-500 transition-all text-sm font-semibold text-slate-800 placeholder-slate-400" placeholder="Masukkan nama lengkap">
                                </div>
                            </div>
                            ...
                            <div>
                                ...
                                <div class="relative">
                                    ...
                                    <input type="email" v-model="form.email" required class="w-full pl-11 pr-4 py-3 rounded-2xl border-2 border-slate-200/70 bg-slate-50 focus:bg-white focus:ring-4 focus:ring-rose-500/10 focus:border-rose-500 transition-all text-sm font-semibold text-slate-800 placeholder-slate-400" placeholder="email@contoh.com">
                                </div>
                            </div>
                            ...
                            <div class="md:col-span-2">
                                ...
                                <div class="relative">
                                    <span class="absolute inset-y-0 left-0 flex items-center pl-4 text-slate-400">🔑</span>
                                    <input type="password" v-model="form.password" class="w-full pl-11 pr-4 py-3 rounded-2xl border-2 border-slate-200/70 bg-slate-50 focus:bg-white focus:ring-4 focus:ring-rose-500/10 focus:border-rose-500 transition-all text-sm font-semibold text-slate-800 placeholder-slate-400" placeholder="Biarkan kosong jika tidak ingin mengubah password">
                                </div>
                            </div>
