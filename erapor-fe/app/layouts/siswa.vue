  ...
  <div class="min-h-screen bg-slate-50 flex text-slate-800 text-sm print:bg-white print:block print:min-h-0">
    ...
    <aside class="group bg-slate-900 text-white flex-shrink-0 min-h-screen hidden lg:flex flex-col fixed lg:static z-50 transition-all duration-300 ease-in-out overflow-x-hidden print:hidden" :class="[sidebarOpen ? 'w-64' : 'lg:w-[72px] lg:hover:w-64']">
      <div class="h-14 flex items-center pl-5 pr-4 bg-slate-950 font-bold text-base tracking-wider border-b border-slate-800 whitespace-nowrap overflow-hidden">
        ...
        <span v-else class="text-indigo-500 mr-1 text-xl shrink-0 cursor-pointer hover:scale-110 transition-transform" @click="visiMisiDialog?.open()">e</span>
      ...
      <div class="p-3">
        <nav class="space-y-1 h-[calc(100vh-140px)] overflow-y-auto custom-scrollbar pr-2">
          <template v-for="(menu, idx) in siswaMenus" :key="idx">
            <div v-if="menu.divider" class="pt-4 pb-1 px-3 text-[10px] font-bold text-indigo-500 uppercase tracking-widest whitespace-nowrap opacity-100 lg:opacity-0 lg:group-hover:opacity-100 transition-opacity duration-300">{{ menu.dividerLabel }}</div>
            <NuxtLink v-else :to="menu.path" class="group flex items-center px-3 py-1.5 text-[13px] font-medium rounded-md text-slate-300 hover:bg-slate-800 hover:text-white transition-colors whitespace-nowrap" active-class="bg-indigo-600 text-white shadow">
      ...
      <header class="relative h-14 bg-white border-b border-slate-200 flex items-center justify-between px-4 sm:px-6 z-[60] shadow-sm flex-shrink-0 print:hidden">
        <div class="flex items-center">
          <div class="lg:hidden flex items-center gap-2 cursor-pointer hover:scale-105 transition-transform" @click="visiMisiDialog?.open()">
            ...
            <span class="font-black text-slate-700 text-sm">e-Rapor <span class="text-indigo-600">Admin</span></span>
          </div>
          <h2 class="hidden lg:block text-base font-bold text-slate-800 ml-3 border-l-2 border-indigo-500 pl-3 py-1 uppercase tracking-wider">{{ route.meta.title || 'Admin Workspace' }}</h2>
        </div>
        <div class="flex-1 lg:flex-none flex justify-end items-center space-x-4">
          <div class="relative">
            <button @click="profileDropdownOpen = !profileDropdownOpen" class="flex items-center space-x-3 text-right focus:outline-none bg-slate-50 hover:bg-slate-100 p-1.5 pl-3 rounded-full border border-slate-200 transition-all">
              ...
              <div class="h-9 w-9 rounded-full bg-gradient-to-br from-indigo-500 to-violet-600 flex items-center justify-center text-white font-black shadow-md text-sm border-2 border-white shrink-0">{{ userInitials }}</div>
            </button>
            <div v-show="profileDropdownOpen" class="absolute right-0 top-full mt-2 w-48 bg-white rounded-xl shadow-xl border border-slate-200 py-2 z-[60] origin-top-right overflow-hidden flex flex-col">
              ...
              <button @click="openProfileModal(); profileDropdownOpen = false" type="button" class="flex items-center px-4 py-2 text-sm text-slate-700 hover:bg-indigo-50 hover:text-indigo-700 transition-colors w-full text-left">
    ...
    <nav class="lg:hidden fixed bottom-0 left-0 right-0 z-50 bg-white border-t border-slate-200 shadow-[0_-4px_20px_-4px_rgba(0,0,0,0.1)] print:hidden">
      <div class="flex items-stretch h-16">
        <NuxtLink to="/siswa/dashboard" class="flex-1 flex flex-col items-center justify-center gap-1 transition-colors" :class="route.path === '/siswa/dashboard' ? 'text-indigo-600' : 'text-slate-400'">
          <span class="text-xl leading-none">📊</span>
          <span class="text-[9px] font-black uppercase tracking-wider">Beranda</span>
        </NuxtLink>
        <button @click="openDrawer('akademik')" class="flex-1 flex flex-col items-center justify-center gap-1 transition-colors" :class="isGroupActive('akademik') ? 'text-indigo-600' : 'text-slate-400'">
          <span class="text-xl leading-none">🎓</span>
          <span class="text-[9px] font-black uppercase tracking-wider">Akademik</span>
        </button>
        <button @click="openDrawer('laporan')" class="flex-1 flex flex-col items-center justify-center gap-1 transition-colors" :class="isGroupActive('laporan') ? 'text-indigo-600' : 'text-slate-400'">
          <span class="text-xl leading-none">📑</span>
          <span class="text-[9px] font-black uppercase tracking-wider">Laporan</span>
        </button>
        <button @click="openDrawer('all')" class="flex-1 flex flex-col items-center justify-center gap-1 transition-colors" :class="isGroupActive('all') ? 'text-indigo-600' : 'text-slate-400'">
          <span class="text-xl leading-none">☰</span>
          ...
          <div class="grid grid-cols-4 gap-y-5 gap-x-3">
            <template v-for="(menu, idx) in currentDrawerMenus" :key="'g-'+idx">
              <NuxtLink v-if="!menu.divider" :to="menu.path" @click="closeDrawer" class="flex flex-col items-center gap-1.5 active:scale-95 transition-transform">
                <div class="w-14 h-14 rounded-2xl flex items-center justify-center text-2xl shadow-sm transition-all" :class="(route.path === menu.path || route.path.startsWith(menu.path + '/')) ? 'bg-indigo-500 shadow-indigo-200 shadow-lg' : 'bg-slate-100'">{{ menu.icon }}</div>
                <span class="text-[9px] font-bold text-center leading-tight w-full" :class="(route.path === menu.path || route.path.startsWith(menu.path + '/')) ? 'text-indigo-700' : 'text-slate-500'">{{ menu.name }}</span>
              </NuxtLink>
            </template>
          </div>
        </div>
...
import { siswaMenus } from '~/utils/menus'
...
const drawerMenuGroups = {
  ...
  laporan: {
    ...
    menus: [
      ...
      { name: 'Kedisiplinan', path: '/siswa/kedisiplinan', icon: '⚖️' },
    ]
  },
  all: {
    title: 'Semua Menu Siswa',
    menus: siswaMenus
  }
}
...
const userInitials = computed(() => {
  if (!userProfile.value?.name) return 'S'
