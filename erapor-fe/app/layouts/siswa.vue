  ...
  <div class="min-h-screen bg-slate-50 flex text-slate-800 text-sm">
    ...
    <aside class="w-64 bg-slate-900 text-white flex-shrink-0 min-h-screen fixed lg:static z-50 transition-transform transform lg:translate-x-0" :class="{ '-translate-x-full': !sidebarOpen }">
      <div class="h-14 flex items-center px-4 bg-slate-950 font-bold text-base tracking-wider border-b border-slate-800">
        ...
        <span class="ml-2 text-[10px] bg-emerald-500/20 text-emerald-300 px-2 py-0.5 rounded border border-emerald-500/30">SISWA</span>
      </div>
      
      <div class="p-3">

        <nav class="space-y-1 h-[calc(100vh-140px)] overflow-y-auto custom-scrollbar pr-2">
          <div class="pt-4 pb-1 px-3 text-[10px] font-bold text-slate-500 uppercase tracking-widest">
            Main Menu
          </div>
          <NuxtLink to="/siswa/dashboard" class="group flex items-center px-3 py-2 text-[13px] font-medium rounded-md text-slate-300 hover:bg-slate-800 hover:text-white transition-colors" active-class="bg-emerald-600 text-white shadow">
            <span class="mr-3 text-lg">🏠</span> Dashboard
          </NuxtLink>

          <NuxtLink to="/siswa/biodata" class="group flex items-center px-3 py-2 text-[13px] font-medium rounded-md text-slate-300 hover:bg-slate-800 hover:text-white transition-colors" active-class="bg-emerald-600 text-white shadow">
            <span class="mr-3 text-lg">🧑‍🎓</span> Biodata Saya
          </NuxtLink>
          
          <NuxtLink to="/siswa/rapor" class="group flex items-center px-3 py-2 text-[13px] font-medium rounded-md text-slate-300 hover:bg-slate-800 hover:text-white transition-colors" active-class="bg-emerald-600 text-white shadow">
            <span class="mr-3 text-lg">📄</span> Rapor Saya
          </NuxtLink>
    ...
    <div class="flex-1 flex flex-col min-w-0 h-screen overflow-hidden">
      ...
      <header class="h-14 bg-white border-b border-slate-200 flex items-center justify-between px-4 sm:px-6 z-10 shadow-sm flex-shrink-0">
        <div class="flex items-center">
          ...
          <h2 class="hidden lg:block text-base font-bold text-slate-800 ml-3 border-l-2 border-emerald-500 pl-3 py-1 uppercase tracking-wider">{{ route.meta.title || 'Siswa Workspace' }}</h2>
        </div>
        ...
        <div class="flex-1 lg:flex-none flex justify-end items-center space-x-4">
          ...
          <div class="flex items-center space-x-3 text-right">
            ...
            <div class="h-9 w-9 rounded-full bg-gradient-to-br from-emerald-500 to-teal-600 flex items-center justify-center text-white font-black shadow-md text-sm border-2 border-white">
              {{ userInitials }}
            </div>
          </div>
