  ...
  <div class="min-h-screen bg-slate-50 flex text-slate-800 text-sm print:bg-white print:block print:min-h-0">
    ...
    <nav class="lg:hidden fixed bottom-0 left-0 right-0 z-50 bg-white border-t border-slate-100 shadow-[0_-10px_20px_-10px_rgba(0,0,0,0.05)] print:hidden">
      <div class="flex items-stretch h-16">
        ...
        <button @click="openDrawer('persiapan')" class="flex-1 flex flex-col items-center justify-center gap-1 transition-colors" :class="isGroupActive('persiapan') ? 'text-slate-800' : 'text-slate-400'">
          <svg xmlns="http://www.w3.org/2000/svg" class="h-[22px] w-[22px]" :class="isGroupActive('persiapan') ? 'stroke-[2.5px] text-amber-500' : 'stroke-[2px]'" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" d="M10.325 4.317c.426-1.756 2.924-1.756 3.35 0a1.724 1.724 0 002.573 1.066c1.543-.94 3.31.826 2.37 2.37a1.724 1.724 0 001.065 2.572c1.756.426 1.756 2.924 0 3.35a1.724 1.724 0 00-1.066 2.573c.94 1.543-.826 3.31-2.37 2.37a1.724 1.724 0 00-2.572 1.065c-.426 1.756-2.924 1.756-3.35 0a1.724 1.724 0 00-2.573-1.066c-1.543.94-3.31-.826-2.37-2.37a1.724 1.724 0 00-1.065-2.572c-1.756-.426-1.756-2.924 0-3.35a1.724 1.724 0 001.066-2.573c-.94-1.543.826-3.31 2.37-2.37.996.608 2.296.07 2.572-1.065z" /><path stroke-linecap="round" stroke-linejoin="round" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z" /></svg>
          <span class="text-[10px] font-bold" :class="isGroupActive('persiapan') ? 'text-slate-800' : ''">Persiapan</span>
        </button>
        <button @click="openDrawer('tugas')" class="flex-1 flex flex-col items-center justify-center gap-1 transition-colors" :class="isGroupActive('tugas') ? 'text-slate-800' : 'text-slate-400'">
          <svg xmlns="http://www.w3.org/2000/svg" class="h-[22px] w-[22px]" :class="isGroupActive('tugas') ? 'stroke-[2.5px] text-amber-500' : 'stroke-[2px]'" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" d="M4 7v10c0 2.21 3.582 4 8 4s8-1.79 8-4V7M4 7c0 2.21 3.582 4 8 4s8-1.79 8-4M4 7c0-2.21 3.582-4 8-4s8 1.79 8 4m0 5c0 2.21-3.582 4-8 4s-8-1.79-8-4" /></svg>
          <span class="text-[10px] font-bold" :class="isGroupActive('tugas') ? 'text-slate-800' : ''">Tugas</span>
        </button>
        <button @click="openDrawer('standar')" class="flex-1 flex flex-col items-center justify-center gap-1 transition-colors" :class="isGroupActive('standar') ? 'text-slate-800' : 'text-slate-400'">
          <svg xmlns="http://www.w3.org/2000/svg" class="h-[22px] w-[22px]" :class="isGroupActive('standar') ? 'stroke-[2.5px] text-amber-500' : 'stroke-[2px]'" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path d="M12 14l9-5-9-5-9 5 9 5z" /><path d="M12 14l6.16-3.422a12.083 12.083 0 01.665 6.479A11.952 11.952 0 0012 20.055a11.952 11.952 0 00-6.824-2.998 12.078 12.078 0 01.665-6.479L12 14z" /><path stroke-linecap="round" stroke-linejoin="round" d="M12 14l9-5-9-5-9 5 9 5zm0 0l6.16-3.422a12.083 12.083 0 01.665 6.479A11.952 11.952 0 0012 20.055a11.952 11.952 0 00-6.824-2.998 12.078 12.078 0 01.665-6.479L12 14zm-4 6v-7.5l4-2.222" /></svg>
          <span class="text-[10px] font-bold" :class="isGroupActive('standar') ? 'text-slate-800' : ''">Standar</span>
        </button>
