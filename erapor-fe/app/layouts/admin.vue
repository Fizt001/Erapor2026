  ...
  <div class="min-h-screen bg-slate-50 flex text-slate-800 text-sm print:bg-white print:block print:min-h-0">
    ...
    <nav class="lg:hidden fixed bottom-0 left-0 right-0 z-50 bg-white border-t border-slate-100 shadow-[0_-10px_20px_-10px_rgba(0,0,0,0.05)] print:hidden">
      <div class="flex items-stretch h-16">
        ...
        <button @click="openDrawer('pengaturan')" class="flex-1 flex flex-col items-center justify-center gap-1 transition-colors" :class="isGroupActive('pengaturan') ? 'text-slate-800' : 'text-slate-400'">
          <svg xmlns="http://www.w3.org/2000/svg" class="h-[22px] w-[22px]" :class="isGroupActive('pengaturan') ? 'stroke-[2.5px] text-emerald-500' : 'stroke-[2px]'" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" d="M10.325 4.317c.426-1.756 2.924-1.756 3.35 0a1.724 1.724 0 002.573 1.066c1.543-.94 3.31.826 2.37 2.37a1.724 1.724 0 001.065 2.572c1.756.426 1.756 2.924 0 3.35a1.724 1.724 0 00-1.066 2.573c.94 1.543-.826 3.31-2.37 2.37a1.724 1.724 0 00-2.572 1.065c-.426 1.756-2.924 1.756-3.35 0a1.724 1.724 0 00-2.573-1.066c-1.543.94-3.31-.826-2.37-2.37a1.724 1.724 0 00-1.065-2.572c-1.756-.426-1.756-2.924 0-3.35a1.724 1.724 0 001.066-2.573c-.94-1.543.826-3.31 2.37-2.37.996.608 2.296.07 2.572-1.065z" /><path stroke-linecap="round" stroke-linejoin="round" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z" /></svg>
          <span class="text-[10px] font-bold" :class="isGroupActive('pengaturan') ? 'text-slate-800' : ''">Sistem</span>
        </button>
        <button @click="openDrawer('master')" class="flex-1 flex flex-col items-center justify-center gap-1 transition-colors" :class="isGroupActive('master') ? 'text-slate-800' : 'text-slate-400'">
          <svg xmlns="http://www.w3.org/2000/svg" class="h-[22px] w-[22px]" :class="isGroupActive('master') ? 'stroke-[2.5px] text-emerald-500' : 'stroke-[2px]'" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" d="M4 7v10c0 2.21 3.582 4 8 4s8-1.79 8-4V7M4 7c0 2.21 3.582 4 8 4s8-1.79 8-4M4 7c0-2.21 3.582-4 8-4s8 1.79 8 4m0 5c0 2.21-3.582 4-8 4s-8-1.79-8-4" /></svg>
          <span class="text-[10px] font-bold" :class="isGroupActive('master') ? 'text-slate-800' : ''">Master</span>
        </button>
        <button @click="openDrawer('akademik')" class="flex-1 flex flex-col items-center justify-center gap-1 transition-colors" :class="isGroupActive('akademik') ? 'text-slate-800' : 'text-slate-400'">
          <svg xmlns="http://www.w3.org/2000/svg" class="h-[22px] w-[22px]" :class="isGroupActive('akademik') ? 'stroke-[2.5px] text-emerald-500' : 'stroke-[2px]'" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path d="M12 14l9-5-9-5-9 5 9 5z" /><path d="M12 14l6.16-3.422a12.083 12.083 0 01.665 6.479A11.952 11.952 0 0012 20.055a11.952 11.952 0 00-6.824-2.998 12.078 12.078 0 01.665-6.479L12 14z" /><path stroke-linecap="round" stroke-linejoin="round" d="M12 14l9-5-9-5-9 5 9 5zm0 0l6.16-3.422a12.083 12.083 0 01.665 6.479A11.952 11.952 0 0012 20.055a11.952 11.952 0 00-6.824-2.998 12.078 12.078 0 01.665-6.479L12 14zm-4 6v-7.5l4-2.222" /></svg>
          <span class="text-[10px] font-bold" :class="isGroupActive('akademik') ? 'text-slate-800' : ''">Akademik</span>
        </button>
        <button @click="openDrawer('all')" class="flex-1 flex flex-col items-center justify-center gap-1 transition-colors" :class="isGroupActive('all') ? 'text-slate-800' : 'text-slate-400'">
          <svg xmlns="http://www.w3.org/2000/svg" class="h-[22px] w-[22px]" :class="isGroupActive('all') ? 'stroke-[2.5px] text-emerald-500' : 'stroke-[2px]'" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" d="M4 6h16M4 12h16M4 18h16" /></svg>
          ...
          <div class="grid grid-cols-4 gap-y-5 gap-x-3">
            <template v-for="(menu, idx) in currentDrawerMenus" :key="'g-'+idx">
              <NuxtLink v-if="!menu.divider" :to="menu.path" @click="closeDrawer" class="flex flex-col items-center gap-1.5 active:scale-95 transition-transform">
                <div class="w-14 h-14 rounded-2xl flex items-center justify-center shadow-sm transition-all text-emerald-600" :class="(route.path === menu.path || route.path.startsWith(menu.path + '/')) ? 'bg-emerald-500 text-white shadow-emerald-200 shadow-lg' : 'bg-slate-50 border border-slate-100'" v-html="getSvgIcon(menu.icon)"></div>
                <span class="text-[9px] font-bold text-center leading-tight w-full" :class="(route.path === menu.path || route.path.startsWith(menu.path + '/')) ? 'text-emerald-700' : 'text-slate-500'">{{ menu.name }}</span>
              </NuxtLink>
            </template>
          </div>
        </div>
...
const getSvgIcon = (emoji) => {
  const icons = {
    ...
    '🗓️': '<svg xmlns="http://www.w3.org/2000/svg" class="h-[22px] w-[22px]" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2"><path stroke-linecap="round" stroke-linejoin="round" d="M8 7V3m8 4V3m-9 8h10M5 21h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v12a2 2 0 002 2z" /></svg>',
    ...
    '📖': '<svg xmlns="http://www.w3.org/2000/svg" class="h-[22px] w-[22px]" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2"><path stroke-linecap="round" stroke-linejoin="round" d="M12 6.253v13m0-13C10.832 5.477 9.246 5 7.5 5S4.168 5.477 3 6.253v13C4.168 18.477 5.754 18 7.5 18s3.332.477 4.5 1.253m0-13C13.168 5.477 14.754 5 16.5 5c1.747 0 3.332.477 4.5 1.253v13C19.832 18.477 18.247 18 16.5 18c-1.746 0-3.332.477-4.5 1.253" /></svg>',
    '💾': '<svg xmlns="http://www.w3.org/2000/svg" class="h-[22px] w-[22px]" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2"><path stroke-linecap="round" stroke-linejoin="round" d="M8 7H5a2 2 0 00-2 2v9a2 2 0 002 2h14a2 2 0 002-2V9a2 2 0 00-2-2h-3m-1 4l-3 3m0 0l-3-3m3 3V4" /></svg>'
  };
