...
# Replace template block accurately
# By using r'<template>.*?</template>\n\n<script setup>', we ensure it matches the outermost template tag.
new_content = re.sub(r'<template>.*?</template>\n\n<script setup>', new_template + '\n\n<script setup>', original, flags=re.DOTALL)
...
print('Updated rapor layout to dock & flow successfully.')
